...
class CreateDatacards(SerializeInferenceModelBase):
    ...
    def run(self):
        ...
        for config_inst in self.config_insts:
            ...
            # step 1: collect histograms per config process
            input_hists: dict[od.Process, hist.Hist] = self.load_process_hists(inputs, cat_obj, config_inst)

            # step 2: apply hist hooks
            input_hists = self.invoke_hist_hooks(config_inst, input_hists)

            # step 3: transform to nested histogram as expected by the datacard writer for the config
